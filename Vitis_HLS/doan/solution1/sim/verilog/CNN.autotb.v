...
parameter LATENCY_ESTIMATION = 1735898;
...
wire [4 - 1 : 0]    arrayInModel_we0, arrayInModel_we1;
wire    [8 : 0]    arrayInModel_address0, arrayInModel_address1;
wire    [31 : 0]    arrayInModel_din0, arrayInModel_din1;
wire    [31 : 0]    arrayInModel_dout0, arrayInModel_dout1;
...
reg AESL_REG_OutModel_ap_vld = 0;
// The signal of port OutModel
reg [31: 0] AESL_REG_OutModel = 0;
always @(posedge AESL_clock)
begin
    if(AESL_reset)
        AESL_REG_OutModel = 0; 
    else if(OutModel_ap_vld) begin
        AESL_REG_OutModel <= OutModel;
        AESL_REG_OutModel_ap_vld <= 1;
    end
end 

initial begin : write_file_process_OutModel
    integer fp;
    integer fp_size;
    integer err;
    integer ret;
    ...
    integer proc_rand;
    integer OutModel_count;
    reg [151:0] token;
    integer transaction_idx;
    reg [8 * 5:1] str;
    wait(AESL_reset === 0);
    fp = $fopen(`AUTOTB_TVOUT_OutModel_out_wrapc,"w");
    if(fp == 0) begin       // Failed to open file
        $display("Failed to open file \"%s\"!", `AUTOTB_TVOUT_OutModel_out_wrapc);
    ...
    while (transaction_idx != AUTOTB_TRANSACTION_NUM) begin
        ...
        if(AESL_REG_OutModel_ap_vld)  begin
          $fdisplay(fp,"0x%x", AESL_REG_OutModel);
        AESL_REG_OutModel_ap_vld = 0;
        end
...
wire [4 - 1 : 0]    arrayWeights_we0, arrayWeights_we1;
...
reg end_OutModel;
reg [31:0] size_OutModel;
reg [31:0] size_OutModel_backup;
