...
if __name__ == '__main__':
    ...
    list_file_name = glob.glob('C:\\Users\\satou\\OneDrive\\Desktop\\Doan1\\mitdb\\*.hea')
    X = np.zeros((0, 4))
    y = np.zeros((0, 1))
    ...
    df_train = pd.DataFrame(np.append(X_train, y_train, axis=1),
                            columns=['filename_lr', 'start', 'end', 'channel', 'labels'])
    df_val = pd.DataFrame(np.append(X_val, y_val, axis=1),
                          columns=['filename_lr', 'start', 'end', 'channel', 'labels'])
    df_test = pd.DataFrame(np.append(X_test, y_test, axis=1),
                           columns=['filename_lr', 'start', 'end', 'channel', 'labels'])

    df_train.to_csv('mit_bih/train_test/train_data.csv', index=False, header=True)
    df_val.to_csv('mit_bih/train_test/val_data.csv', index=False, header=True)
    df_test.to_csv('mit_bih/train_test/test_data.csv', index=False, header=True)
